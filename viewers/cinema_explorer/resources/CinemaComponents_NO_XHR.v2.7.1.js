...
(function() {
	 ...
	 */
	CINEMA_COMPONENTS.Database = function(directory, csvs, callback, errorCallback, filter) {
        ...
        // CYRUSH: HACK
        parseCSVText(self.csvs.data, function(data_arr) {
            ...
            self.loaded = true;
            self.hasAxisOrdering = false;
            if (callback)
                callback(self);

            //Attempt to load an axis_order.csv file
            if( "axis_order" in self.csvs )
            {
                parseCSVText(self.csvs.axis_order,
	...
	CINEMA_COMPONENTS.Database.prototype.refreshData = function(reloadAllData) {
        ...
        // if (reloadAllData) {
            ...
            //     self.errorCallback);
            /// OLD END
            parseCSVText(self.csvs.main,
                         /* callback */
                         function(data_arr, request)
                         {
                             dataUpdateCallback(self, data_arr);
                         },
                         /* error callback */
                         self.errorCallback);
        // }
	 ...
	 */
    // CYRUSH: FIX THIS
	var dataUpdateCallback = function(self, data_arr) {
	...
	var parseCSVText = function(csv_text, callback, errorCallback) {
        // var request = new XMLHttpRequest();
        // request.open("GET",path,true);
        // request.onreadystatechange = function() {
        //     if (request.readyState === 4) {
